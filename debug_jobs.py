...
def main():
    ...
    parser.add_argument("--mode", choices=["raw", "unique", "target", "ts"], default="raw",
                        help="raw=scope後, unique=scope+unique後, target=ターゲット抽出, ts=企業×月×国で条件一致した全行を全列でCSV出力")
...
def _ts_mode(conn: sqlite3.Connection, args):
    """指定企業・指定月・指定国に合う全行を全列のまま CSV 出力"""
    ...
    """
    params = [company_pattern, f"{country} %", month_start, month_end]
    rows = conn.execute(sql, params).fetchall()

    if not rows:
        print(f"結果: 0 件 (company×{month_start}×{country})", file=sys.stderr)
        return
